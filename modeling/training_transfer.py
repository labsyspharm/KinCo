...
this script trains a model with one set of hyperparameters specified under 
$exptindex/$runindex
"""
import numpy as np
...
import tfbio.net_noReLU_v1 as net_noReLU
from tfbio.net_transfer_v1 import make_transfer_network
...
tr_group.add_argument('--fine_tune', default='False', type=str,
                      help='whether to freeze imported layers')
tr_group.add_argument('--random_seed',  type=int,
                      help='random seed to control stochasticity')
tr_group.add_argument('--meta_path', type=str,
...
print('training imported layers: ', args.fine_tune)
...
ref_cvset = pd.read_csv('/data/cl321/Datasets/D13/dataset/PDBBIND-kinase_DTC_cvsplit1127.csv')
...
# import network
graph = make_transfer_network(meta_path = meta_path, fine_tune= args.fine_tune,
                           isize=isize, in_chnls=in_chnls, osize=osize,
...
net_summaries, training_summaries = tfbio.net_transfer_v1.make_summaries_SB(graph)

x = graph.get_tensor_by_name('pafnucy/input/structure:0')
y = graph.get_tensor_by_name('transfer/output/prediction:0')
t = graph.get_tensor_by_name('pafnucy/input/affinity:0')
keep_prob = graph.get_tensor_by_name('transfer/fully_connected/keep_prob:0') # uses the transfer network
train = graph.get_tensor_by_name('transfer/training/train:0') # uses the transfer network
mse = graph.get_tensor_by_name('transfer/training/mse:0') # uses the transfer network
feature_importance = graph.get_tensor_by_name('net_properties/feature_importance:0')
global_step = graph.get_tensor_by_name('transfer/training/global_step:0')
...
with graph.as_default():
    ...
    var_extract = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='pafnucy/convolution')

    var_dict = {}
    for var in var_extract:
        if 'optimizer' not in var.name:
            var_dict[var.name[8:-2]] = var
#    print(var_dict) 
    pafnucy_saver = tf.train.Saver(var_list = var_dict)
...
def get_predictions(val_or_test, steps):
    ...
    merge.to_csv('{0}/{1}_{2}_prediction.csv'.format(args.output_prefix[:-6], val_or_test, int(steps)),
                 index=False)
...
test_sample = min(args.batch_size, len(features['test']))
...
with tf.Session(graph=graph, config = config) as session:
    ...
    for epoch in range(current_epoch, args.num_epochs):
        ...
        get_predictions('validation', global_step.eval())
        # test set error
        get_predictions('test', global_step.eval())
